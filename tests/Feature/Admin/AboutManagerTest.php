...
it('saves multilingual about content and uploads the hero image to public', function () {
    $component = Livewire::test(AboutManager::class);
    $firstPrincipleKey = $component->get('repeaterOrder.principles.0');

    $component
        ->set('translations.tr.headline', 'Yeni hakkımda başlığı')
        ->set('translations.en.headline', 'New about headline')
        ->set("principles.tr.{$firstPrincipleKey}.text", 'Önce doğru soruyu sor.')
        ->set("principles.en.{$firstPrincipleKey}.text", 'Ask the right question first.')
        ->set('heroImage', UploadedFile::fake()->image('about-wide.jpg', 1600, 600))
    ...
    expect($setting->getTranslation('headline', 'tr'))->toBe('Yeni hakkımda başlığı')
        ->and($setting->getTranslation('principles', 'en')[0]['text'])->toBe('Ask the right question first.')
        ->and($setting->hero_image_path)->toStartWith('images/about/uploads/')
        ->and(public_path($setting->hero_image_path))->toBeFile();
...
it('imports the complete demo about content idempotently', function () {
    ...
    expect(AboutSetting::count())->toBe(1)
        ...
        ->and($setting->getTranslation('principles', 'en'))->toBe(config('about.principles.en'))
        ->and($setting->hero_image_path)->toBe(config('about.hero_image_path'))
